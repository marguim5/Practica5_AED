...
#ordenamos cada variable con arrange() del paquete dyplr 
dirty_iris %>% arrange(Sepal.Length) 
dirty_iris %>% arrange(Sepal.Width) 
dirty_iris %>% arrange(Petal.Length) 
dirty_iris %>% arrange(Petal.Width)

#de mayor a menor con desc()

dirty_iris %>% arrange(desc(Sepal.Length))
dirty_iris %>% arrange(desc(Sepal.Width)) 
dirty_iris %>% arrange(desc(Petal.Length)) 
dirty_iris %>% arrange(desc(Petal.Width))
```

Podemos observar valores extraños, lo primero que nos llama la atención es la presencia de varios NA. Lo siguiente, resultado de ordenar los datos nos percatamos de varias anomalías:

- Valores extrañamente grandes para las partes del sépalo y pétalo de una flor (lirio), por ejemplo: 63.000 en Petal.Lenght (podría ser un outlier) o incluso Inf en Petal.Width
- Valores imposibles como, por ejemplo, 0 en Petal.Lenght o -3 en Sepal Width.
...
dirty_iris <- dirty_iris %>%  mutate(
  #Utilizo across para recorrerme todas las columnas
  across(
    #Donde la variable sea numerica aplico este filtrado
    # la virgulilla (~) sirve para declarar una función anónima 
    # el punto (.) para indicar dónde va la variable que le meto a la función anónima
    where(is.numeric), ~ ifelse(!is.finite(.)|.<=0,NA,.)
    )
  )
summary(dirty_iris)

```
Como podemos gracias a `summary()`, hemos eliminado los valores nefativos,
ceros y valores especiales como es el Inf.
...
#Numero de observaciones completas
numero<-sum(complete.cases(dirty_iris))
numero
#Porcentaje de observaciones completas
porcentaje<-numero/nrow(dirty_iris)*100
print(paste(round(porcentaje,2),"%"))

#Nuevo data frame limpio

clean_iris<-dirty_iris %>% na.omit()

```
El porcentaje no es tan elevado, en un caso real no podríamos eliminar esas observaciones, deberíamos imputar los datos faltantes.
...
```{r , include = TRUE}
#aplicamos summary a las variables numericas
clean_iris %>% summarise(
...
#Y ahora para el dirty

dirty_iris %>% summarise(
  ...
  across(
    ...
    c(
      ...
      media = mean(. ,na.rm=T),
      desviacion_estandar = sd(. ,na.rm=T),
...
```
Obtenemos valores distintos ya que en el clean tenemos datos completos, pero en el dirty tenemos más datos.

Para la MADM hemos utilizado `mad` pero se nos pide que la construyamos nosotros también
```{r}
#Función MADM
MADM<- function (columna){
  return(median(abs(columna-median(columna))*1.4826))
}
#comprobamos que da igual
mad(clean_iris$Petal.Length)
MADM(clean_iris$Petal.Length)
```
...
tsigma<-function(x){
    ...
    sigma<-sd(x)
    media<-mean(x)
    outlier<-x[abs(x-media)>3*sigma]

  
    return(outlier)
}

tsigma(clean_iris$Petal.Length)
```
...
Hampel<-function(x){
  ...
  M<-mad(x)
  mediana<-median(x)
  outlier<-x[abs(x-mediana)>3*M]
  
  return(outlier)
}

Hampel(clean_iris$Petal.Length)
```
...
BoxPlot<- function(x){
  ...
  sup<-x_s+1.5*IQR
  inf<-x_i-1.5*IQR
  outliers<-x[x>sup|x<inf]
  return(outliers)
}
BoxPlot(clean_iris$Petal.Length)
```
...
Percentiles<- function(x){
  Percentiles<-quantile(x, probs = c(0.05,0.95))
  x_s<-Percentiles[2]
  x_i<-Percentiles[1]
  outliers<-x[x>x_s|x<x_i]
  return(outliers)
}

Percentiles(clean_iris$Petal.Length)
